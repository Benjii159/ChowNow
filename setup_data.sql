...
SELECT
	  *
	, LEAD(order_id)  OVER (PARTITION BY customer_id ORDER BY order_date DESC) AS next_order_id
	, LEAD(product)   OVER (PARTITION BY customer_id ORDER BY order_date DESC) AS next_product
	, LEAD(price)     OVER (PARTITION BY customer_id ORDER BY order_date DESC) AS next_price
